...
CREATE TABLE IF NOT EXISTS `cine_colombia`.`Persona` (
  `Documento_Pesona` INT NOT NULL COMMENT 'Numero de identificacion de persona',
...
CREATE TABLE IF NOT EXISTS `cine_colombia`.`Rol` (
  `Persona_Documento_Pesona` INT NOT NULL,
  `Contrasena` VARCHAR(500) NOT NULL,
  `Cod_Rol` VARCHAR(45) NOT NULL COMMENT 'Nombre el rol',
  PRIMARY KEY (`Persona_Documento_Pesona`, `Cod_Rol`),
  INDEX `fk_Rol_Persona1_idx` (`Persona_Documento_Pesona` ASC),
  CONSTRAINT `fk_Rol_Persona1`
    FOREIGN KEY (`Persona_Documento_Pesona`)
    REFERENCES `cine_colombia`.`Persona` (`Documento_Pesona`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
